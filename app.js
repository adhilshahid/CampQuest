...
app.use(passport.initialize()) // This is a middleware function that initializes Passport for authentication
app.use(passport.session()) // This is a middleware function that manages the session for Passport authentication
passport.use(new LocalStrategy(User.authenticate())) // This is where we configure Passport to use the LocalStrategy for authentication. The User.authenticate() method is provided by the passport-local-mongoose package and is used to authenticate users using their username and password.
passport.serializeUser(User.serializeUser()) // This is a method that serializes the user object into the session. It is used to store the user ID in the session.
